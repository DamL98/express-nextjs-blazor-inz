...
export function CancelReservationDialog({ reservation, busy, error, onClose, onConfirm }: CancelReservationDialogProps) {
  ...
  return (
    <dialog ref={dialogRef} onCancel={(event) => {
      ...
      <h2 id="cancel-title" className="mt-3 text-2xl font-bold"><AppIcon name="calendar-x" />Anulować tę rezerwację?</h2>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
        ...
        <button type="button" disabled={busy} onClick={onConfirm} className="rounded-xl bg-red-700 px-4 py-3 text-sm font-semibold text-white hover:bg-red-800"><AppIcon name="calendar-x" />{busy ? "Anulowanie…" : "Anuluj rezerwację"}</button>
      </div>
