import { beginOAuthFlow, consumeOAuthFlow } from "./oauth-flow.service.js";
...
export function applySessionCookie(res, token) {
  res.cookie(
...
export async function startGoogleLink(_req, res) {
  ...
  return ApiResponse.ok({ authorizationUrl: buildGoogleAuthorizationUrl({ state }) }).send(res);
}
